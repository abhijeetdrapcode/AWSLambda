...
const replaceProp = (
  ...
  if (prop.startsWith('DF::')) {
    ...
    // Replacing Current User Data
    let cleanProp = prop.replace('current_user.', '');
    if (cleanProp.startsWith('DF::')) {
      // Replacing Current User Derived Field
      cleanProp = cleanProp.replace('DF::', '');
      const derivedField = userCollection.utilities.find((field) => field.name === cleanProp);
      dataOfItem = prepareFunction(
        derivedField,
        user,
        user,
      ...
      dataOfItem = parseValueFromData(user, cleanProp);
      const { fields } = userCollection;
    ...
    // Replacing Current Tenant Data
    let cleanProp = prop.replace('current_tenant.', '');
    if (cleanProp.startsWith('DF::')) {
      // Replacing Current Tenant Derived Field
      cleanProp = cleanProp.replace('DF::', '');
      const derivedField = tenantCollection.utilities.find((field) => field.name === cleanProp);
      dataOfItem = prepareFunction(
        derivedField,
        tenant,
        user,
      ...
      dataOfItem = parseValueFromData(tenant, cleanProp);
      const { fields } = tenantCollection;
      const field = fields.find((field) => field.fieldName === prop);
      if (field && field.type === 'boolean') {
        dataOfItem = dataOfItem ? 'Yes' : 'No';
      }
...
export const sendDynamicEmailService = async (req, isDeveloperApi = false) => {
  ...
  try {
    ...
    if (user && user.uuid) {
      const userData = await findItemById(db, projectId, userCollection, user.uuid);
    ...
    for (const templateRule of templatesRules) {
      ...
      try {
        ...
        let itemDataOfTemplateCollection = collectionName
          ? (await findItemById(db, projectId, collection, itemIdForTemplate))?.data || {}
          : {};
