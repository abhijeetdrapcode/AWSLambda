...
import {
  ...
  extractFirstUserSettingIdFromUser,
  getTenantById,
  getUserSettingById,
...
export const addUserWithProvider = async (req, res, next) => {
  try {
    const { db, builderDB, body, projectId, params, environment } = req;
...
export const addAnonymousUser = async (req, res, next) => {
  try {
    const { db, builderDB, body, projectId } = req;
    const response = await saveAnonymousUser(builderDB, db, projectId, body);
    return res.status(response.code).send(response.data);
  } catch (err) {
    next(err);
  }
  ...
  passport.authenticate(authType, { session: false }, (err, user, info) => {
    ...
    if (user) {
      req.login(user, { session: false }, async (err) => {
        ...
        const userSettingId = extractFirstUserSettingIdFromUser(userDetails);
...
export const updateTenantPermission = async (req, res) => {
  const { params, projectId, builderDB, db, body } = req;
  try {
...
export const updateUserPermission = async (req, res) => {
  const { params, projectId, builderDB, db, body } = req;
  try {
    const user = await updateUserPermissionsService(builderDB, db, projectId, params.userId, body);
...
export const loginUserWithFacebook = async (req, res, next) => {
  ...
  try {
    const { projectId, builderDB, db, body } = req;
    const { params, accessToken } = body;
    ...
    if (user) {
      ...
      const tenantId = extractFirstTenantIdFromUser(userDetails);
      console.log('\n Before extractFirstUserSettingIdFromUser \n');
      const userSettingId = extractFirstUserSettingIdFromUser(userDetails);
      console.log('\n Before getUserSettingById \n');
      const userSetting = await getUserSettingById(builderDB, db, projectId, userSettingId);
      console.log('\n Before getTenantById \n');
      const tenant = await getTenantById(builderDB, db, projectId, tenantId);
...
export const generateAndSendEmailOTP = async (req, res, next) => {
  try {
    const { body, db, builderDB, projectId, headers, environment, tenant } = req;
    const { email, emailTemplate, otpAuthenticationType, newUserRole } = body;
    if (!email) {
...
export const verifyEmailOtpAndLogin = async (req, res, next) => {
  try {
    const { body, db, builderDB, projectId, headers, environment } = req;
...
export const generateAndSendSmsOTP = async (req, res, next) => {
  try {
    const { body, db, builderDB, projectId, headers, environment, tenant } = req;
    const { phone_number, smsTemplate, otpAuthenticationType, newUserRole } = body;
    if (!phone_number) {
