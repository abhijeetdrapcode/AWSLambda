...
export const processSingleDocument = async (file, retryCount = 1) => {
  ...
  for (let attempt = 1; attempt <= retryCount; attempt++) {
    try {
      ...
      console.warn(`Processing attempt ${attempt} failed for file: ${file.originalname}`, error);

      if (attempt === retryCount) {
        throw new Error(
          `Failed to process file "${file.originalname}" after ${retryCount} attempts. ` +
            `Error: ${error.message}`,
        );
      }
      const delay = 1000 * attempt;
      console.info(`Retrying file: ${file.originalname} after ${delay / 1000} seconds.`);
